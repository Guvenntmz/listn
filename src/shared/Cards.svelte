<script>
    ...
    const handleDelete = async (e) => {
        document.querySelector(`.deleting-spinner-${e.target.id}`).classList.remove('d-none');
                    ...
                    <div class='card-header text-center'>
                        ...
                        {#if component === 'library'}
                            <button class='btn btn-close d-inline-block float-right' id={playlist.id} on:click={handleDelete}>
                                <br>
                                    <span class="spinner-border spinner-border-sm d-none deleting-spinner-{playlist.id} mt-3 text-danger" role="status" aria-hidden="true"></span>
                            </button>
                        {/if}
